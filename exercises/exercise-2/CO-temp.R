...
## ---- fit_spLM, fig.align="center", fig.width=10---------------------------------------------------------------------------
...
## ---- map_both_models_random_effects---------------------------------------------------------------------------------------
...
base.map %>%
    ...
    addRasterImage(w.m1.surf, colors = pal, opacity = 0.75, group="spLM m.1") %>%
    addRasterImage(resid.surf, colors = pal, opacity = 0.75, group="Regression residuals") %>%
    addLegend("bottomleft", pal = pal.2, values = values(w.m2.surf), opacity = 0.75, title = "<center>Random effects (m.2)</center>") %>%
